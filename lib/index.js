...
import { isClient, isServer, window, document } from "./platform";
...
class Popover extends React.Component {
    static defaultTransition = {
        ...
        transitionTimingFunction: 'cubic-bezier(0.23, 1.00, 0.32, 1.00)',
        opacity: '1',
    ...
    static defaultProps = {
        ...
        theme: {
            ...
            container: 'Popover',
            tip: 'Popover-tip',
            transition: Popover.defaultTransition,
            zoneHeight: 50,
        },
        tipped: false,
        tipSize: 7,
    ...
    resolvePopoverLayout = () => {
        ...
        this is thus: First position cross-axis self to the cross-axis-center of the the targePropTypes. Then,
        offset self by the amount that self is past the boundaries of frame. */
        const pos = Layout.calcRelPos(zone, tb, this.size);

        /* Offset allows users to control the distance betweent the tip and the targePropTypes. */
        ...
        frameEl. `frameBuffer` length is based on tipSize and its offsePropTypes. */
        ...
        if (pos.crossLength > frameCrossLength) {
            ...
            log("popoverCrossStart cannot hang any further without losing targePropTypes.");
        ...
        /* So far the link position has been calculated relative to the targePropTypes. To calculate the absolute
    ...
    open() {
        if (this.state.exiting && !this.props.disableAnimations)
            this.animateExitStop();
    ...
    animateExit() {
        ...
        this.exitingAnimationTimer2 = setTimeout(() => {
            setTimeout(() => {
                this.containerEl.style.transform = `${flowToPopoverTranslations[this.zone.flow]}(${this.zone.order * this.props.theme.zoneHeight}px)`
    ...
    animateEnter() {
        ...
        const { transition } = this.props.theme;

        for (const key of Object.keys(transition)) {
            this.containerEl.style[key] = transition[key];
        }
    }
    ...
    trackPopover() {
        ...
        this.containerEl = ReactDOM.findDOMNode(this.layerReactComponent);
        this.bodyEl = this.containerEl.querySelector(`.${theme.body}`);
        this.tipEl = this.containerEl.querySelector(`.${theme.tip}`);
        ...
        /* Watch for boundary changes in all deps, and when one of them changes, recalculate layouPropTypes.
        ...
        /* Kickstart layout at first booPropTypes. */
    ...
    renderLayer() {
        ...
        const classes = `${theme.container} ${className}`;
    ...
    _layerUnrender() {
        if (this.layerWillUnmount)
            this.layerWillUnmount(this.layerContainerNode);
